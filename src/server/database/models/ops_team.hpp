...
public:
    ...
    DEFINE_FIELD(is_valid, sqlpp::boolean);
    ...
    GET_FIELD_H(std::uint32_t, armored); // Number of armored vehicles deployed
    GET_FIELD_H(std::uint32_t, battle_gear);  // Is battle gear deployed
    GET_FIELD_H(std::uint32_t, car);          // Number of cars deployed
    GET_FIELD_H(std::uint32_t, combat_count); // Deployed Main Combat Unit Size
    GET_FIELD_H(std::uint32_t,
    ...
    GET_FIELD_H(std::uint32_t, seed); // Seems to be 0 regardless of if it's an
                                      // active team or not, and doesn't match
    GET_FIELD_H(std::uint32_t, staff_power); // Total Fighting Ability for this
                                             // team (How it calculates Success
    GET_FIELD_H(std::uint32_t, sub_count);   // Deployed Supporting Unit Size
    GET_FIELD_H(std::uint32_t,
    ...
    GET_FIELD_H(std::uint32_t,
                sub_rank_top); // The highest rank deployed (see Staff Ranks
                               // (take rank and add 1))
    GET_FIELD_H(std::uint32_t, tank);       // Number of tanks deployed
    GET_FIELD_H(std::uint32_t, team_power); // Seems to match staff_power
    GET_FIELD_H(std::uint32_t, time);  // Time (in seconds) left of the mission
    GET_FIELD_H(std::uint32_t, truck); // Number of Trucks deployed
    GET_FIELD_H(std::uint32_t, walker_gear); // Number of walker gear deployed
    GET_FIELD_H(std::uint32_t,
